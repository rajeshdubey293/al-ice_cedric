...
BYTE bEDID_twoblock[256];
EXPORT_SYMBOL(bEDID_twoblock);

extern int EDID_ready;
...
void SP_TX_Initialization(struct VideoFormat* pInputFormat)
{
	...
	*/
	...
	sp_write_reg(SP_TX_PORT0_ADDR, SP_TX_DEBUG_REG1, (c|0x80));//disable polling HPD, force hotplug for HDCP, disable polling
	#endif
...
BYTE get_bandwidth_and_pclk(void)
{
	...
	if(SP_TX_Video_Input.Interface == LVTTL_RGB) {
		...
		pclk = mipi_video_timing_table[bMIPIFormatIndex].MIPI_pixel_frequency/1000;
		...
		if(mipi_lane_count>4 ||mipi_lane_count<1  )
		{
...
void SP_TX_AudioInfoFrameSetup(struct AudioFormat *bAudioFormat)
{
	...
	SP_TX_AudioInfoFrmae.version = 0x00;
	SP_TX_AudioInfoFrmae.length = 0x00;

	if(bAudioFormat ->bAudioType ==AUDIO_I2S) {
		switch(bAudioFormat->bI2S_FORMAT.Channel_Num) {
		case I2S_CH_2:
			SP_TX_AudioInfoFrmae.type=0x01;//coding type ,refer to stream header, audio channel count,two channel
			SP_TX_AudioInfoFrmae.pb_byte[0]=0x00;//for multi channel LPCM
			break;
		case I2S_CH_4:
			SP_TX_AudioInfoFrmae.type=0x03;
			SP_TX_AudioInfoFrmae.pb_byte[0]=0x08;
			break;
		case I2S_CH_6:
			SP_TX_AudioInfoFrmae.type=0x05;
			SP_TX_AudioInfoFrmae.pb_byte[0]=0x0b;
			break;
		case I2S_CH_8:
			SP_TX_AudioInfoFrmae.type=0x07;
			SP_TX_AudioInfoFrmae.pb_byte[0]=0x13;
			break;
		default:
			break;
		}
		...
		SP_TX_AudioInfoFrmae.type=0x00;//coding type ,refer to stream header, audio channel count,two channel
		SP_TX_AudioInfoFrmae.pb_byte[0]=0x00;//for multi channel LPCM
	}
...
void SP_TX_AUX_POLLING_ENABLE(BYTE benable)
{
	...
	sp_write_reg(SP_TX_PORT0_ADDR, SP_TX_DEBUG_REG1, c);
}
...
BYTE SP_TX_AUX_DPCDRead_Bytes(BYTE addrh, BYTE addrm, BYTE addrl,BYTE cCount,BYTE * pBuf)
{
	...
	if(!bOK) {
		...
		{
			sp_read_reg(SP_TX_PORT2_ADDR, SP_TX_INT_STATUS1, &c);
			sp_read_reg(SP_TX_PORT0_ADDR, SP_TX_DEBUG_REG1, &c1);
			//if polling is enabled, wait polling error interrupt
			if(c1&POLLING_EN){
				if(c & SP_TX_INT_STATUS1_POLLING_ERR)
					SP_TX_RST_AUX(); //to reset AUX, for AUX failed;
			}else{
				SP_TX_RST_AUX();
			}
		}
...
BYTE SP_TX_HW_Link_Training (void)
{
	...
	case LINK_TRAINING_FINISH:
		...
		if((ByteBuf[0]&0x07) != 0x07)// work with ANX7732
...
void SP_TX_Config_MIPI_Video_Format()
{
	...
	*/
	...
	sp_read_reg(SP_TX_PORT2_ADDR, 0x11, &c);
...
BYTE sp_tx_check_sink_connection(void)
{
	...
	for (i = 0; i < Sink_DPCD_Count; i++) {
		if(AUX_ERR == SP_TX_AUX_DPCDRead_Bytes(0x00, 0x00, 0x00, 1, &c)) {
			connection--;
...
#ifdef Standard_DP
	SP_TX_AUX_DPCDWrite_Bytes(0x00, 0x02, DPCD_DEVICE_SERVICE_IRQ_VECTOR, 4, ByteBuf);
	if ((!(ByteBuf[3] & 0x01))||((ByteBuf[1] & 0x07) != 0x07))
	{
		if((sp_tx_system_state > SP_TX_LINK_TRAINING )
			   &&sp_tx_link_training_state > LINK_TRAINING_PRE_CONFIG) {
			SP_CTRL_Set_System_State(SP_TX_LINK_TRAINING);
...
void SP_CTRL_EDID_Process(void)
{
	...
	for (i = 0; i < 128; i++)
		bEDID_twoblock[i] = bEDID_firstblock[i];
